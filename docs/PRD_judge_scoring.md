...
(Ex §9). A judge who "knows the right answer" would be biased; withholding the
domain truth keeps scoring on rhetoric, evidence use, and rebuttal quality.

## 2. Specific requirements
- **No tie, ever** (Ex §8.4). Output a single `winner` and a **differential
...
opponent. Drives `intervene()` (Ex §9: Father must re-assert the role).
...
- *Alt: numeric auto-scoring from heuristics only* — rejected: assignment requires
  a real LLM judgment of persuasion, not a Python formula (Ex §8.4).
